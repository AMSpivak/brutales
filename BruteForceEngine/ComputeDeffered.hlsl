...
Texture2D<float4> depth : register(t8);
...
void main(ComputeShaderInput IN)
{
    ...
    if(IN.DispatchThreadID.x < w && IN.DispatchThreadID.y < h)
    {
        //if (IN.DispatchThreadID.x & 1)
        {
            ...
            float3 Normal_smpl = textures[materials.r * material_offset + 1].SampleGrad(sampl, UV.xy, Ddx_Ddy.xy, Ddx_Ddy.zw).xyz;
            Normal_smpl.y = -Normal_smpl.y;
            ...
            sun_light *= 0.2 + clamp(sun_light_diffuse, 0.0, 1.0);// *shadows.x;// *shadows.x;
