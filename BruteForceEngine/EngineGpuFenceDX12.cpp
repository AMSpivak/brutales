...
namespace BruteForce
{
	namespace Sync
	{
		...
		{
			ThrowIfFailed(commandQueue->Wait(fence.Get(), fenceValue));
		}
		...
		{
			return BruteForce::Sync::Signal(commandQueue, m_fence, m_fence_value);
		}

		void SmartFence::GpuWait(CommandQueue commandQueue)
		{
			BruteForce::Sync::GpuWait(commandQueue, m_fence, m_fence_value);
		}
		void SmartFence::GpuWaitNext(CommandQueue commandQueue)
		{
			BruteForce::Sync::GpuWait(commandQueue, m_fence, m_fence_value + 1);
		}
		...
		{
			BruteForce::Sync::WaitForFenceValue(m_fence, fenceValue, m_handle, duration);
		}
	}
}