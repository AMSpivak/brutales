...
namespace Character
{
    ...
    class BrainHorde : public IBrain
    {
    private:
    ...
    public:
        BrainHorde(std::function<void(GlCharacter& character)> world_reaction) : rotator(0),
            distance(0.0f),
            step_back_distance(2.0f),
            walk_distance(7.0f),
            attak_distance(3.3f),
    ...
    std::shared_ptr<IBrain> CreateBrain(BrainTypes brain_type, std::function<void(GlCharacter & character)> world_reaction)
    {
        switch(brain_type)
        {
            ...
            case BrainTypes::Horde:
                return std::make_shared<BrainHorde>(world_reaction);
            default:
    ...
    std::default_random_engine BrainHorde::random_generator;
}