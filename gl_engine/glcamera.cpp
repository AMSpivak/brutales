...
namespace GlScene
{
	...
	const glm::vec3& glCamera::GetFrustrumNormal(FrustrumNormals normal) const
	{
		return Normals[static_cast<int>(normal)];
	}
	...
	void glCamera::RecalculateFrustrum()
    {
		...
		auto vflu = Frustrum[static_cast<int>(FrustrumPoints::FarLU)];
		auto vfld = Frustrum[static_cast<int>(FrustrumPoints::FarLD)];
		auto vnlu = Frustrum[static_cast<int>(FrustrumPoints::NearLU)];

		auto vfru = Frustrum[static_cast<int>(FrustrumPoints::FarRU)];
		auto vfrd = Frustrum[static_cast<int>(FrustrumPoints::FarRD)];
		auto vnrd = Frustrum[static_cast<int>(FrustrumPoints::NearRD)];		
		Normals[static_cast<int>(FrustrumNormals::Far)] = glm::normalize(glm::cross(glm::vec3(vfru - vflu), glm::vec3(vfrd - vflu)));
		Normals[static_cast<int>(FrustrumNormals::Near)] = -Normals[static_cast<int>(FrustrumNormals::Far)];

		Normals[static_cast<int>(FrustrumNormals::Left)] = glm::normalize(glm::cross(glm::vec3(vflu - vnlu), glm::vec3(vfld - vnlu)));
		Normals[static_cast<int>(FrustrumNormals::Right)] = glm::normalize(glm::cross(glm::vec3(vfrd - vnrd), glm::vec3(vfru - vnrd)));
		Normals[static_cast<int>(FrustrumNormals::Up)] = glm::normalize(glm::cross(glm::vec3(vfru - vnlu), glm::vec3(vflu - vnlu)));
		Normals[static_cast<int>(FrustrumNormals::Down)] = glm::normalize(glm::cross(glm::vec3(vfld - vnrd), glm::vec3(vfrd - vnrd)));
